...
public class AddManaToItemCommand {
        ...
        dispatcher.register(Commands.literal("addmana")
                ...
                .then(Commands.argument("amount", IntegerArgumentType.integer(1))
                        .executes(context -> {
                            ...
                            ManaUtils.setCurrentMana(heldItem, ManaUtils.getCurrentMana(heldItem) + amount);
                            player.getCapability(PlayerManaProvider.PLAYER_MANA).ifPresent(mana -> {
                                mana.updateManaFromInventory(player);
                            });
                            player.displayClientMessage(Component.literal("Восстановлено " + amount + " маны!"), true);
                            return 1;
                        })
                )
        );
